...
class QuestionGenerator:
    ...
    def generate_mcq(self, topic: str, difficulty: str = 'medium') -> MCQuestion:
        try:
            ...
            question = self.retry_and_parse(mcq_prompt_template, parser, topic, difficulty)
            ...
            self.logger.error(f"Faileed to generate MCQ: {str(e)}")
            raise CustomException("MCQ generation failed", e)
        
    def generate_fill_blank(self, topic: str, difficulty: str='medium') -> FillBlankQuestion:
        try:
            parser = PydanticOutputParser(pydantic_object=FillBlankQuestion)
            question = self.retry_and_parse(fill_blank_prompt_template, parser, topic, difficulty)
            if "______" not in question.questions:
                raise ValueError("Fill in blanks should contain '______'")
            self.logger.info("Generated a valid Fill in Blanks Question")
            return question 
        except Exception as e:
            self.logger.error(f"Failed to generate fill ups: {str(e)}")
